...
int16_t GitRepo::getReleases(uint8_t num) {
  ...
  sprintf(url, "https://api.github.com/repos/piv3rt/espsomfy-rts/releases?per_page=%d&page=1", count);
...
int GitUpdater::checkInternet() {
  ...
  HTTPClient https;
  https.setReuse(false);
  if(https.begin(sclient, "https://github.com/piv3rt/ESPSomfy-RTS")) {
...
bool GitUpdater::beginUpdate(const char *version) {
  ...
  if(strcmp(version, "Main") == 0)  strcpy(this->baseUrl, "https://raw.githubusercontent.com/piv3rt/ESPSomfy-RTS/master/");
  else sprintf(this->baseUrl, "https://github.com/piv3rt/ESPSomfy-RTS/releases/download/%s/", version);
...
bool GitUpdater::recoverFilesystem() {
  sprintf(this->baseUrl, "https://github.com/piv3rt/ESPSomfy-RTS/releases/download/%s/", settings.fwVersion.name);
  strcpy(this->currentFile, "SomfyController.littlefs.bin");
  this->status = GIT_UPDATING;
  this->partition = U_SPIFFS;
  this->lockFS = true;
